...
import com.zx.gamarketmobile.view.MultiSpinner;
import com.zx.gamarketmobile.view.SimpleSpinnerOption;
...
public class SuperviseMyTaskExcuteActivity extends BaseActivity {
    ...
    private void initData() {
        ...
        if (getIntent().hasExtra("entity")) {
            ...
            taskState = mEntity.getFTaskStatus();
            radioButton0.setChecked(true);
            if (taskState.equals(State.TOBEISSUED.getName())) {//待下发 进去进行下发操作
            ...
            } else if (taskState.equals(State.TOBEAUDIT.getName())) {//待审核  进去执行审核操作
            ...
            }else if (taskState.equals(State.TOBEASSIGNMETN.getName())) {//待指派 执行指派操作
                topLayout.setVisibility(View.GONE);
                opitionLayout.setVisibility(View.GONE);
                spinnerLayout.setVisibility(View.VISIBLE);
                getProcessingPeople.setLoadingListener(this);
                getProcessingPeople.loadData(userInfo.getId());
            }else if (taskState.equals(State.TOBEFEEDBACK.getName())) {//待反馈
                topLayout.setVisibility(View.GONE);
                opitionLayout.setVisibility(View.VISIBLE);
                spinnerLayout.setVisibility(View.GONE);
                feedbackTask.setLoadingListener(this);
            }else if (taskState.equals(State.TOBEREVIEW.getName())) {//待核审
    ...
    private void intiView() {
        ...
        radioButton0.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked){
                    if (taskState.equals(State.TOBEISSUED.getName())){//待下发
                        getProcessingPeople.loadData(userInfo.getId());
                    }
                }
            }
               ...
               if (taskState.equals(State.TOBEISSUED.getName())){//待下发
                   if (radioButton0.isChecked()){//自办
                       ...
                       if (mProcessingPeopleList.size()>0){
                           ...
                           if (options!=null&&options.size()>=0){
                               ...
                               sendTask.loadData(mEntity.getFTaskId(),"",userId,userInfo.getId(),ids,names);
                           }
                       }
                       ...
                       if (mutiSpinner.getCheckedOptions().size()>0){
                           ...
                           sendTask.loadData(mEntity.getFTaskId(),userId,"",userInfo.getId(),"","");
                       }else{
                           showToast("请选择下发单位！");
                       }

                   }
               }else if (taskState.equals(State.TOBEAUDIT.getName())){//待审核
                   if (radioButton0.isChecked()){//未通过
                       examineTaskInfo.loadData(0,mEntity.getFTaskId(),editText.getText().toString(),userInfo.getId());
                   }else if (radioButton1.isChecked()){//通过
                       examineTaskInfo.loadData(1,mEntity.getFTaskId(),editText.getText().toString(),userInfo.getId());
                   }
               }else if (taskState.equals(State.TOBEASSIGNMETN.getName())){//待指派
                   String userId = getProcessingPeopleId(spinner.getSelectedItem().toString());
                   ...
                   if (mProcessingPeopleList.size()>0){
                       ...
                       if (options!=null&&options.size()>=0){
                           ...
                           assignTaskInfo.loadData(userId,mEntity.getFTaskId(),userInfo.getId(),ids,names);
                       }
                   }

               }else if (taskState.equals(State.TOBEREVIEW.getName())){//待核审
                   if (radioButton0.isChecked()){//回退
                       reviewTask.loadData(mEntity.getF_GUID(),0,editText.getText().toString(),userInfo.getId());
                   }else if (radioButton1.isChecked()){//通过
                       reviewTask.loadData(mEntity.getF_GUID(),1,editText.getText().toString(),userInfo.getId());
                   }
               }else if (taskState.equals(State.TOBEFEEDBACK.getName())){//待反馈
                   feedbackTask.loadData(mEntity.getF_GUID(),mEntity.getFTaskId(),editText.getText().toString(),userInfo.getId());
               }
            }
        });
    }
